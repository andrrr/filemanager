<?php
	...
	Class extension_filemanager extends Extension{
		...
		public function about(){
			return array('name' => __('File Manager'),
						 'version' => '0.9',
						 'release-date' => '2010-01-31',
						 'author' => array('name' => 'Symphony Team',
										   'website' => 'http://symphony-cms.com',
										   'email' => 'team@symphony-cms.com'),
						 'description' => __('Upload, Edit and manage files and folders.')
				 		);
		}
		...
		public function savePreferences($context){
			...
			$context['settings']['filemanager'] = array(
														  ...
														  'archive-name' => (isset($conf['archive-name']) ? $conf['archive-name'] : __('Backup'))
														);
		...
		public function appendPreferences($context){
			...
			$group->appendChild(new XMLElement('legend', __('File Manager')));


			$label = Widget::Label(__('Root Browse Location'));
			...
			$group->appendChild(new XMLElement('p', __('This path is relative to the root Symphony installation folder, <code>%s</code>', array(DOCROOT)), array('class' => 'help')));

			$label = Widget::Label(__('Archive Name'));
			...
			$group->appendChild($label);
			$group->appendChild(new XMLElement('p', __('Default filename for archives generated by File Manager'), array('class' => 'help')));
			...
			$label->setValue($input->generate() . __(' Show Hidden Files and Folders'));
			$group->appendChild($label);

			$group->appendChild(new XMLElement('p', __('Hidden files will not be included in archives unless this is checked.'), array('class' => 'help')));
		...
		public function download($file){
			...
			if(!file_exists($file))
				$this->_Parent->customError(E_USER_ERROR, 'File Not Found', __('The file you requested, <code>%s</code>, does not exist.', $file), false, true, 'error', array('header' => 'HTTP/1.0 404 Not
Found'));
		...
		public function buildTableRow(DirectoryIterator $file, $includeParentDirectoryDots=true){
			...
			if(!$file->isDot()){
				...
				if($file->isWritable()) {
					if($file->isDir()){
						$td5 = Widget::TableData(Widget::Anchor(__('Edit'), $download_uri));
					} else {
						$td5 = Widget::TableData(Widget::Anchor(__('Download'), $download_uri));
					}
				}
				else {
